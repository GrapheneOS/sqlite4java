...
public class _SQLiteManual {
  ...
  public static SWIGTYPE_p_sqlite3 sqlite3_open_v2(String filename, int flags, int[] returnCode) {
    long[] ppDb = {0};
    int rc = _SQLiteManualJNI.sqlite3_open_v2(filename, ppDb, flags);
    if (returnCode != null) {
  ...
  public static SWIGTYPE_p_sqlite3_stmt sqlite3_prepare_v2(SWIGTYPE_p_sqlite3 db, String sql, int[] returnCode) {
    long[] ppStmt = {0};
    int rc = _SQLiteManualJNI.sqlite3_prepare_v2(SWIGTYPE_p_sqlite3.getCPtr(db), sql, ppStmt);
    if (returnCode != null) {
      if (returnCode.length == 1) {
        returnCode[0] = rc;
      } else {
  ...
  public static String sqlite3_column_text(SWIGTYPE_p_sqlite3_stmt stmt, int column, int[] returnCode) {
    ...
    int rc = _SQLiteManualJNI.sqlite3_column_text(SWIGTYPE_p_sqlite3_stmt.getCPtr(stmt), column, ppResult);
    if (returnCode != null) {
      if (returnCode.length == 1) {
        returnCode[0] = rc;
      } else {
